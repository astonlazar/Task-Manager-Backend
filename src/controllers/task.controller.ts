...
export default class TaskController {
  ...
  getAllTasks = async (req: Request, res: Response) => {
    try {
      ...
      const filter: any = {};
      if (status && ["todo", "ongoing", "done"].includes(status as string)) {
  ...
  createTask = async (req: Request, res: Response) => {
    try {
      ...
      const newTask = new TaskModel({ title, description, status: "todo" })
  ...
  changeStatus = async (req: Request, res: Response) => {
    try {
      ...
      if (!["todo", "ongoing", "done"].includes(status as string)) {
        return res.status(HTTP.BAD_REQUEST).json({ message: "Status issue" })
      }
